...
- Para **Create Incoming Invoices** (Fiori): tab por tab — Basic Data (vendor, doc type, dates, amount, tax), Payment (terms), líneas G/L (account, cost center, amount)
- Tax amount: si el enunciado dice "X euro plus Y% VAT" → ingresar el monto neto + marcar Calculate Tax + código de IVA correcto. El sistema calcula el impuesto automáticamente.
- Siempre indicar con qué transacción verificar el resultado: **FB03** para documentos FI
- En FB03 verificar: número de documento, Assignment field (Sort Key), Document Header Text, número externo en Document Number
- ⚠️ Si el enunciado pide "use the new document type" → asegurarse de seleccionarlo explícitamente en el header del documento
...
- ¿En OBA7: marqué los Account Types correctos (K+S para vendor, D+S para customer)?
- ¿En FBN1: especifiqué Internal vs External? ¿Year correcto (actual vs 9999)? ¿Mencioné el bridge step de vuelta a OBA7?
- ¿En BP: cubrí Reconciliation Account + Sort Key + Payment Terms + las tabs correctas?
- ¿En Document Splitting: usé BT 0300 para vendor o BT 0200 para customer?
- ¿En la task de testing: detallé todos los campos incluyendo dónde va el número externo?
